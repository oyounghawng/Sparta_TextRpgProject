...
namespace Sparta_TextRpg
{
    internal class DOHYUN_BattleScene : BaseScene
    {
        ...
        private void ViewPotion()
        {
            ...
            {
                Console.WriteLine($"{i + 1}. {inventory[i]._name} 회복량 : {inventory[i]._statvalue} 보유갯수 : {inventory[i]._cnt}");
            }
        ...
        {
            Item inventory = player._inventory.FirstOrDefault(item => item._itemtype == ItemType.POTION);

            if (inventory != null)
            {
                if (inventory._cnt >= 1)
                {
                    player._currenthp += inventory._statvalue;
                    inventory._cnt--;

                    Console.WriteLine(inventory._name);
                }
            }
            else
            {
